...
# module_1_limma_norm <- create_module(biofeaturesNames=gene_names, normalization = "limma")
```
...
# get_sampleset_id(name_In = "GSM605594.ch1-vs-GSM605578.ch1")
knitr::kable(get_sampleset_id(name_In = colnames(module_1), version="legacy"))
# knitr::kable(get_sampleset_id(name_In = colnames(module_1)[1:10], version="latest"))
```
...
```{r plot_module_heatmap, eval=FALSE, include=FALSE}
plot_module_heatmap(module = module_1, normalization = "legacy", plot = TRUE)
# plot_module_heatmap(module = module_1_limma, normalization = "limma", plot = TRUE)
```
or

```{r plot_module_heatmap_html, eval=FALSE, include=FALSE}
my_plot_html <- plot_module_heatmap(module = module_1, type = "html", plot = FALSE)
...
```{r heatmaply, eval=FALSE, include=FALSE}
...
```{r pheatmap_module1, eval=FALSE, include=FALSE}
...
```{r pheatmap order, eval=FALSE, include=FALSE}
...
```{r plot_module_network, eval=FALSE, include=FALSE}
my_plot_html <- plot_module_network(module = module_1, type = "html", threshold = 0.7, plot = FALSE)
h <- xml2::read_html(my_plot_html)
tempDir <- tempfile()
dir.create(tempDir)
htmlFile <- file.path(tempDir, "plot_network.html")
xml2::write_html(h, tmpDir,file = htmlFile)
rstudioapi::viewer(htmlFile)
```
...
```{r plot_module_distribution, eval=FALSE, include=FALSE}
plot_module_distribution(module = module_1, plot = TRUE)
```
```{r plot_module_distribution html, eval=FALSE, include=FALSE}
my_plot_html <- plot_module_distribution(module = module_1, type = "html", plot = FALSE)
h <- xml2::read_html(my_plot_html)
tempDir <- tempfile()
dir.create(tempDir)
htmlFile <- file.path(tempDir, "plot_dist.html")
xml2::write_html(h, tmpDir,file = htmlFile)
rstudioapi::viewer(htmlFile)
```
...
```{r}
top_30_bf <- get_biofeature_ranking(samplesetNames =colnames(module_1), top_n = 30)
# top_30_bf_ids = ranked_bf['id'][:30]
# top_30_bf = BiologicalFeature.using(compendium).get(filter={'id_In': top_30_bf_ids})
```
...
```{r}
# write_table(module_1, file ="module_1.txt", sep ="\t")
# module_1  <- read.table("module_1,txt")
module_2 <- create_module(biofeaturesNames= top_30_bf$id[1:10], normalization = "legacy", useIds = TRUE)
#mod_union <- merge_modules(mod1 = module_1, mod2 = module_2)
```

We can have a look at the heatmap of the new module

```{r pheatmap_module_2, eval=FALSE, include=FALSE}
pheatmap::pheatmap(mat = module_2)
```
...
module_3 <- create_module_ss(samplesetNames = seed_ss$id, useIds = TRUE)
```

And now we can plot the heatmap

```{r plot_heatmap_module_3, eval=FALSE, include=FALSE}
my_plot_html <- plot_module_distribution(module = module_3, type = "html", plot = FALSE)
h <- xml2::read_html(my_plot_html)
tempDir <- tempfile()
dir.create(tempDir)
htmlFile <- file.path(tempDir, "plot_heat3.html")
xml2::write_html(h, tmpDir,file = htmlFile)
rstudioapi::viewer(htmlFile)
pheatmap::pheatmap(mat = as.matrix(module_3))
```
# Session information
