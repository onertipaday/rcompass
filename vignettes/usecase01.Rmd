...
view_plot(module = module_1, plotType = "module_heatmap_expression", min = -6, max = 6)
```
...
```{r}
top_bf <- get_biofeature_ranking(samplesetNames = colnames(module_1), top_n = 50)
# top_bf_ids = ranked_bf['id'][:30]
# top_bf = BiologicalFeature.using(compendium).get(filter={'id_In': top_bf_ids})
```
...
module_2 <- create_module(biofeaturesNames= top_bf$name[1:10], normalization = "legacy", useIds = FALSE)
