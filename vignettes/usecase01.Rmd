...
or we can use the wrapper function `get_ids_from_alias()`:
...
**CHECK THIS!**
Another examples returning samples ids (**they are not**)selected by [PO ontology](https://www.ebi.ac.uk/ols/ontologies/po) term [seed](http://purl.obolibrary.org/obo/PO_0009010):
...
get_sparql_annotation_triples(target = "sample", query = my_query)[,1]
```
...
knitr::kable(get_sampleset_id(name_In = colnames(module_1)[1:10]))
```
# Plot a module
...
library(pheatmap)
pheat_cl <- pheatmap(mat =  as.matrix(module_1))
pheat_cl
```
