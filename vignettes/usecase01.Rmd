...
```{r}
unsorted_ids <- get_sampleset_id(name_In = colnames(module_1), version="legacy")
#get_sampleset_id(name_In = my_sorted_indexes$sortedSamplesets, useIds = TRUE)
unsorted_ids[match(my_sorted_indexes$sortedSamplesets,unsorted_ids$id),]
```
...
Alternatively we can use variables containing the order for both rows and columns produced by the other heatmap functions.
...
my_plot_html <- plot_module_network(module = module_1, type = "html", plot = FALSE)
h <- xml2::read_html(my_plot_html)
tempDir <- tempfile()
dir.create(tempDir)
