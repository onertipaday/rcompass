...
```{r}
module_limma <- module_1
colnames(module_limma) <- get_sampleset_id(normalization = "limma", id_In = colnames(module_limma), useIds = T)$name
row.names(module_limma) <- get_biofeature_id(id_In = rownames(module_limma))$name
knitr::kable(module_limma, class = 'cell-border stripe')
```
...
```{r}
module_2 <- create_module(biofeaturesNames = c(rownames(module_1), top_bf$id[1:15]), 
                             normalization = "limma", 
                             useIds = TRUE)
```
...
```{r}
seed_ss <- get_sampleset_id(id_In = seed_samples, useIds = TRUE)
module_3 <- create_module(samplesetNames = seed_ss$id, useIds = TRUE)
```
