...
and check the results are the same:

```{r}
identical(sparql_results[,1],sparql_results_check)
```

Another examples returning samples ids selected by [PO ontology](https://www.ebi.ac.uk/ols/ontologies/po) term [seed](http://purl.obolibrary.org/obo/PO_0009010):
```{r}
...
```{r}
# module_3 <- create_module(samplesetNames = seed_samples, useIds = TRUE)
```
