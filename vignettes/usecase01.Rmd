...
```{r}
DT::datatable(get_biofeature_by_name(name_In = gene_names))
```
...
```{r}
DT::datatable(get_biofeature_by_name(name_In = unique(unique( get_annotation_triples(ids = sparql_results[,1])[,1]))))
```
...
```{r}
DT::datatable(module_1, class = 'cell-border stripe')
```
```{r}
# get_sample_by_gsm(sampleName_Icontains="GSM1313535")
# get_sampleset_id(name_In = "GSM605594.ch1-vs-GSM605578.ch1")
DT::datatable(get_sampleset_id(name_In = colnames(module_1)[1:10]))
```
...
pheat_cl
```
Biological Features (genes) and Sample Sets (contrasts) have no particular order in a module. But since the heatmap is clustered we can retrieve the sorted list of Biological Features and Sample Sets respectively. 

```{r}
my_sorted_indexes <- plot_module_heatmap(module = module_1, plot = FALSE, sorted = TRUE)
```

Alternatively we can use variables containing the order for both rows and columns produced by the `pheatmap` function.

```{r}
DT::datatable(module_1[pheat_cl$tree_row$order,pheat_cl$tree_col$order])
```
...
module_2 <- create_module(biofeaturesNames= top_30_bf$id[1:10], version = "legacy", useIds = TRUE)
...
```{r}
# plot_module_heatmap(module = module_2, plot = TRUE)
```
