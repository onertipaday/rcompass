...
# fig <- plot_ly(x =as.vector(module_df)[1:1000],
#              type = "histogram",
#              histnorm = "probability density")
# fig %>% add_trace(x = ~ trace_1, name = "lowly expressed",  mode = "lines", col = "red") %>% layout(xaxis = list(title = 'Week'), yaxis = list(title = 'Source'))
...
GO_plant_slim <- read.table("/home/paolo/Sandbox/GO_plant_slim_fixed.tab", sep="\t", header=FALSE, stringsAsFactors=FALSE)
...
( resultFisher.MF <- getSigGroups(GOdata.MF, test.stat) )
( resultFisher.CC <- getSigGroups(GOdata.CC, test.stat) )
( resultFisher.BP <- getSigGroups(GOdata.BP, test.stat) )

topNodes = 20
resultFis.MF <- runTest(GOdata.MF, statistic = "fisher") # weight01 is the default algorithm 
allRes.MF <- data.frame( GenTable(GOdata.MF, classic = resultFis.MF, orderBy = "weight", 
	ranksOf = "classic", topNodes = topNodes), Ontology=rep("MF", topNodes) )
resultFis.CC <- runTest(GOdata.CC, statistic = "fisher") # weight01 is the default algorithm 
allRes.CC <- data.frame( GenTable(GOdata.CC, classic = resultFis.CC, orderBy = "weight", 
	ranksOf = "classic", topNodes = topNodes), Ontology=rep("CC", topNodes) )
...
# write.table(rbind(allRes.MF, allRes.CC, allRes.BP), file=paste("topGO_comuniInd_ripening.txt"), quote=F, row.names=FALSE, sep="\t")
# pdf("GO_comuniInd_ripening.pdf")
showSigOfNodes(GOdata.MF, score(resultFis.MF), firstSigNodes = 10, useInfo = "all")
showSigOfNodes(GOdata.CC, score(resultFis.CC), firstSigNodes = 10, useInfo = "all")
...
```{r custom_annotation_enrichment, results='hide'}
library(GOfuncR)
